...
local ASSET_ID = 125889169899075
...
local function isKnuckles()
	local model = getPlayerModel()
	return model and model:GetAttribute("Character") == "Knuckles"
end
...
local function setupViewport()
	task.spawn(function()
		...
		local function replaceViewportModel()
			local ok, objects = pcall(game.GetObjects, game, "rbxassetid://" .. 125889169899075)
